...
var fetcher$1 = /*@__PURE__*/getAugmentedNamespace(fetcher);
...
async function getNodes() {
  return new Promise(async (resolve, reject) => {
    try {
      const data = await fetcher$1.getFiles().then((e) => {
        return e;
      });
...
function mapNodes() {
  return new Promise((resolve, reject) => {
    try {
      getNodes().then((frame) => {
        console.log(
          `${source.green.bold(`Success`)}: Fetched ${frame.length} frames\n`
        );
        const icons = frame.map((node) => {
          console.log(
            `${source.bold(node.name)} - ${node.children.length} icons`
          );
        ...
        spicymkdir.mkdirSync(outputDir);
        fs.writeFile(
